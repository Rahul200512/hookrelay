...
- [x] **v3 — performance and cold start, measured.** Virtual threads against a platform-thread pool on a 200 ms receiver: 158/s against 70/s, with the latency and dispatch-spread figures that explain why. A GraalVM native image starts in 307 ms against the JVM's 5,058 ms and holds 241 MB against 416 MB, built weekly in CI and smoke-tested afterwards, because a binary that starts fast and cannot serve a signed delivery proves nothing.
